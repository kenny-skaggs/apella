<template>
  <div>
    <div class="my-trumbowyg-custom-icons-container" style="display: none"> <!-- set a class to apply style below -->
      <svg xmlns="http://www.w3.org/2000/svg">
        <symbol id="trumbowyg-question" viewBox="0 0 32 32">
          <g xmlns="http://www.w3.org/2000/svg">
            <g id="question_x5F_mark">
              <g>
                <path style="fill:#030104;" d="M17.6,30c0,1.102-0.895,2-2,2s-2-0.898-2-2c0-1.109,0.895-2,2-2S17.6,28.891,17.6,30z"/>
                <path style="fill:#030104;" d="M15.676,25.977c-1.336,0-2.59-0.523-3.535-1.469c-0.945-1.105-1.465-2.359-1.465-3.695     s0.52-2.59,1.465-3.371l6.688-6.688C19.584,9.996,20,8.992,20,7.926c0-1.07-0.416-2.074-1.172-2.828     c-1.559-1.559-4.096-1.562-5.654,0C12.416,5.852,12,6.855,12,7.926H8c0-2.137,0.834-4.148,2.348-5.66     c3.02-3.023,8.285-3.02,11.309,0.004C23.168,3.777,24,5.785,24,7.926c0,2.137-0.832,4.145-2.344,5.656l-6.688,6.523     c-0.389,0.391-0.389,1.023,0,1.414c0.391,0.391,1.023,0.391,1.414,0c0.254-0.258,0.293-0.555,0.293-0.711h4     c0,1.336-0.52,2.594-1.465,3.699C18.266,25.453,17.012,25.977,15.676,25.977L15.676,25.977z"/>
              </g>
            </g>
          </g>
        </symbol>
      </svg>
    </div>
    <div :id='this.wizzyId'></div>
  </div>
</template>

<script>
import { debounce } from 'vue-debounce';

import 'trumbowyg';
import 'trumbowyg/dist/ui/trumbowyg.css'
import icons from 'trumbowyg/dist/ui/icons.svg'

import 'trumbowyg/dist/plugins/colors/ui/trumbowyg.colors.min.css'
import 'trumbowyg/dist/plugins/colors/trumbowyg.colors.min'
import 'trumbowyg/dist/plugins/fontfamily/trumbowyg.fontfamily.min'
import 'trumbowyg/dist/plugins/fontsize/trumbowyg.fontsize.min'
import 'trumbowyg/dist/plugins/history/trumbowyg.history.min'
import 'trumbowyg/dist/plugins/indent/trumbowyg.indent.min'
import 'trumbowyg/dist/plugins/mathml/ui/trumbowyg.mathml.min.css'
import 'trumbowyg/dist/plugins/mathml/trumbowyg.mathml.min'
import 'trumbowyg/dist/plugins/table/ui/trumbowyg.table.min.css'
import 'trumbowyg/dist/plugins/table/trumbowyg.table.min'
import buildQuestionPlugin from '@/components/editor/trumbowyg.question.js'

let v_editor_component_handle;

export default {
  name: 'HtmlEditor',
  props: {
    value: String,
    pageId: Number,
    allowQuestionInsert: {
      type: Boolean,
      default: false
    }
  },
  data() {
    return {
      currentPageId: undefined,
      currentPageHtml: '',
      wizzyId: this.$store.getters.nextUniqueId()
    }
  },
  methods: {
    htmlChanged() {
      this.$emit('input', $(this.wizzySelector).html());
    },
    questionClicked(event, questionElement) {
      this.$emit('questionClicked', questionElement);
    }
  },
  mounted() {
    v_editor_component_handle = this;

    $.trumbowyg.svgPath = icons;
    MathJax.Hub.Config({
      tex2jax: {
        inlineMath: [
            ['$', '$'],
            ['\\(', '\\)']
        ]
      }
    });

    let insertPluginsAvailable = ['link', 'table', 'insertImage', 'mathml'];
    if (this.allowQuestionInsert) {
      buildQuestionPlugin($);
      insertPluginsAvailable.push('question');
    }

    $(this.wizzySelector).trumbowyg({
      autogrow: true,
      btns: [
          ['viewHTML'],
          ['historyUndo', 'historyRedo'],
          ['formatting', 'fontfamily', 'fontsize'],
          ['strong', 'em', 'del'],
          ['foreColor', 'backColor'],
          ['superscript', 'subscript'],
          ['indent', 'outdent'],
          insertPluginsAvailable,
          ['justifyLeft', 'justifyCenter', 'justifyRight', 'justifyFull'],
          ['unorderedList', 'orderedList'],
          ['horizontalRule'],
          ['removeformat']
      ]
    });
    $(this.wizzySelector).html(this.value);
    this.currentPageId = this.pageId;

    $(this.wizzySelector).on('tbwchange', this.htmlChanged);

    if (this.allowQuestionInsert) {
      $(this.wizzySelector).on('question-clicked', this.questionClicked);
    }
  },
  computed: {
    wizzySelector() {
      return `#${this.wizzyId}`;
    }
  },
  watch: {
    value(new_html) {
      // used to prevent refreshes of the editor when the html is emitted to the parent
      if (this.pageId !== this.currentPageId) {
        this.currentPageId = this.pageId
        $(this.wizzySelector).html(new_html);
      }
    }
  }
}
</script>